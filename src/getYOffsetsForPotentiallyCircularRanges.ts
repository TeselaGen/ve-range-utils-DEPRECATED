...
import { getYOffsetForPotentiallyCircularRange, YOffsetLevelsWithRangesType } from "./getYOffsetForPotentiallyCircularRange";
export function getYOffsetsForPotentiallyCircularRanges(ranges: AnnRange[], assignYOffsetToRange?: boolean) {
    //adjust the yOffset of the range being pushed in by checking its range against other ranges already in the row
    const yOffsets: number[] = [];
    let maxYOffset = 0;
    const yOffsetLevels: YOffsetLevelsWithRangesType = [] //yOffsetLevels is an array of arrays (array of yOffset levels holding arrays of ranges)
